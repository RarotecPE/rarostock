...
export function EquipmentTab() {
  ...
  const filtered = useMemo(() => {
    ...
    if (holderFilter) {
      result = result.filter((equipment) => holderFilter === "company" ? equipment.holderType === "company" : equipment.holderUserId === holderFilter);
    }
    if (categoryFilter) result = result.filter((equipment) => equipment.category === categoryFilter);
    return result;
  }, [equipments, search, holderFilter, categoryFilter]);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
        ...
        {canMutateStock ? <button className="rounded-lg bg-blue-600 px-4 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-blue-500" type="button" onClick={() => setModalOpen(true)}>Novo equipamento</button> : null}
      </div>
      <div className="grid grid-cols-1 gap-3 lg:grid-cols-[1fr_220px_220px]">
        ...
        <select value={holderFilter} onChange={(e) => setHolderFilter(e.target.value)} className="rounded-lg border border-slate-700 bg-slate-800 px-3 py-2.5 text-white outline-none focus:ring-2 focus:ring-blue-500"><option value="">Todos os portadores</option><option value="company">RAROTEC</option>{users.map((item) => <option key={item.id} value={item.id}>{item.nome}</option>)}</select>
        <select value={categoryFilter} onChange={(e) => setCategoryFilter(e.target.value)} className="rounded-lg border border-slate-700 bg-slate-800 px-3 py-2.5 text-white outline-none focus:ring-2 focus:ring-blue-500"><option value="">Todas as categorias</option>{categories.map((category) => <option key={category.id} value={category.name}>{category.name}</option>)}</select>
      </div>

...
export function EquipmentTab() {
  ...
  const load = useCallback(async () => {
    ...
    if (!equipmentRes.ok) {
      setToast({ message: "NÃ£o foi possÃ­vel carregar os equipamentos.", type: "error" });
    }
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
        <div className="text-center lg:text-left">
          ...
          <p className="mt-1 text-sm text-slate-400">Controle quem estÃ¡ com cada equipamento da empresa.</p>
        </div>
...
function EquipmentFormModal({ equipment, categories, onClose, onDone }: { equipment?: Equipment; categories: Array<{ id: number; name: string }>; onClose: () => void; onDone: () => void }) {
  ...
  const save = async () => {
    ...
    if (!draft.code.trim() || !draft.name.trim() || !draft.category.trim()) {
      setError("CÃ³digo, nome e categoria sÃ£o obrigatÃ³rios.");
      return;
    }
    setSaving(true);
    ...
    setSaving(false);
    if (!res.ok) {
      setError(payload?.error ?? "NÃ£o foi possÃ­vel salvar.");
      return;
    }
    onDone();
  };
      ...
      <div className="w-full max-w-2xl rounded-2xl border border-slate-800 bg-slate-900 p-6">
        <div className="mb-5 flex justify-between">
          <div>
            ...
            <p className="text-sm text-slate-400">CÃ³digo patrimonial obrigatÃ³rio e Ãºnico.</p>
          </div>
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          ...
          <Input label="CÃ³digo *" value={draft.code} onChange={(value) => setDraft({ ...draft, code: value })} />
          ...
          <Input label="PreÃ§o" value={draft.price} type="number" onChange={(value) => setDraft({ ...draft, price: value })} />
...
export function EquipmentDetailModal({ equipment, users, currentUserId, isAdmin, canEdit, onEdit, onClose, onDone }: { equipment: Equipment; users: EquipmentUser[]; currentUserId: string; isAdmin: boolean; canEdit: boolean; onEdit?: () => void; onClose: () => void; onDone: (message: string) => void }) {
  ...
  const actionLabel = isAvailable ? "Obter" : isMine ? (targetUserId === "company" ? "Devolver" : "Transferir") : "Solicitar transferÃªncia";
  const submit = async () => {
...
function TransferTargetPreview({ targetUserId, user }: { targetUserId: string; user: EquipmentUser | null }) {
  ...
  const name = isCompany ? "RAROTEC" : user?.nome ?? "UsuÃ¡rio selecionado";

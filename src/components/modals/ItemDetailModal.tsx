...
type ChartStats = { min: number; average: number; max: number } | null;
...
export function ItemDetailModal({ item, onClose, onUpdate, canManageStock, isAdmin = false }: Props) {
  ...
  const valueChartStats = useMemo(() => calculateChartStats(filteredValueChartData), [filteredValueChartData]);
  const stockChartStats = useMemo(() => calculateChartStats(filteredStockChartData), [filteredStockChartData]);
...
function calculateChartStats(data: ProductChartPoint[]): ChartStats { const values = data.map((point) => point.value).filter((value) => Number.isFinite(value) && value !== 0); if (!values.length) return null; const total = values.reduce((sum, value) => sum + value, 0); return { min: Math.min(...values), average: total / values.length, max: Math.max(...values) }; }
...
function ProductLineChart({ title, data, stats, color, unit, units, kind }: { title: string; data: ProductChartPoint[]; stats: ChartStats; color: string; unit: string; units: Parameters<typeof pluralizeUnit>[2]; kind: "stock" | "value" }) { const labelsByKey = new Map(data.map((point) => [point.xKey, point.label])); return <div><h3 className="mb-3 text-sm font-medium text-slate-300">{title}</h3><div className="h-44 rounded-lg bg-slate-800/50 px-2 py-3">{data.length ? <ResponsiveContainer width="100%" height="100%"><LineChart data={data} margin={{ top: 8, right: 12, bottom: 0, left: -22 }}><CartesianGrid stroke="#334155" strokeDasharray="3 3" vertical={false} /><XAxis dataKey="xKey" tick={{ fill: "#94a3b8", fontSize: 11 }} tickLine={false} axisLine={{ stroke: "#475569" }} minTickGap={18} tickFormatter={(value) => labelsByKey.get(String(value)) ?? ""} /><YAxis tick={{ fill: "#94a3b8", fontSize: 11 }} tickLine={false} axisLine={false} tickFormatter={(value) => String(value)} /><Tooltip cursor={{ stroke: "#64748b", strokeWidth: 1 }} content={({ active, payload, label }) => <ProductChartTooltip active={active} payload={payload as unknown as ReadonlyArray<{ payload?: ProductChartPoint }> | undefined} label={String(label ?? "")} unit={unit} units={units} kind={kind} />} /><Line type="monotone" dataKey="value" stroke={color} strokeWidth={2.5} dot={{ r: 3, stroke: color, strokeWidth: 2, fill: "#0f172a" }} activeDot={{ r: 5 }} /></LineChart></ResponsiveContainer> : <div className="flex h-full items-center justify-center text-sm text-slate-500">Sem dados para exibir</div>}</div><ChartStatsSummary stats={stats} kind={kind} unit={unit} units={units} /></div>; }
function ChartStatsSummary({ stats, kind, unit, units }: { stats: ChartStats; kind: "stock" | "value"; unit: string; units: Parameters<typeof pluralizeUnit>[2] }) { const format = (value: number) => kind === "value" ? `R$ ${value.toFixed(2)}` : `${Number.isInteger(value) ? value : value.toFixed(1)} ${pluralizeUnit(unit, Math.round(value), units)}`; return <div className="mt-3 grid grid-cols-3 gap-2 rounded-lg border border-slate-800 bg-slate-950/35 p-2 text-center"><div><p className="text-[10px] uppercase tracking-wider text-slate-500">Mínimo</p><p className="mt-0.5 text-xs font-semibold text-slate-200">{stats ? format(stats.min) : "—"}</p></div><div><p className="text-[10px] uppercase tracking-wider text-slate-500">Média</p><p className="mt-0.5 text-xs font-semibold text-slate-200">{stats ? format(stats.average) : "—"}</p></div><div><p className="text-[10px] uppercase tracking-wider text-slate-500">Máximo</p><p className="mt-0.5 text-xs font-semibold text-slate-200">{stats ? format(stats.max) : "—"}</p></div></div>; }
